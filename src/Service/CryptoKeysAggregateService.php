...
namespace BlackCat\Database\Packages\CryptoKeys\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\CryptoKeys\Dto\CryptoKeyDto;
use BlackCat\Database\Packages\CryptoKeys\Mapper\CryptoKeyDtoMapper;
 ...
 */
final class CryptoKeysAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private CryptoKeyRepository $cryptoKeyRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'crypto_keys' => $this->cryptoKeyRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\CryptoKeys\ModuleException("Record {$idStr} not found for locking.");
            }
