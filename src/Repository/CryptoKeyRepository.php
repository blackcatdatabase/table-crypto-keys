...
namespace BlackCat\Database\Packages\CryptoKeys\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\CryptoKeys\Definitions;
use BlackCat\Database\Packages\CryptoKeys\Criteria;
use BlackCat\Database\Packages\CryptoKeys\Dto\CryptoKeyDto as Dto;
use BlackCat\Database\Packages\CryptoKeys\Mapper\CryptoKeyDtoMapper as RowMapper;
...
final class CryptoKeyRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\CryptoKeys\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'filename', 'file_path', 'fingerprint', 'key_meta', 'key_type', 'algorithm', 'length_bits', 'origin', 'usage', 'scope', 'status', 'is_backup_encrypted', 'backup_blob', 'created_by', 'activated_at', 'retired_at', 'replaced_by', 'notes' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'basename', 'version' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\CryptoKeys\Definitions::table());
            return $bulk->upsertMany($rows, [ 'basename', 'version' ], [ 'filename', 'file_path', 'fingerprint', 'key_meta', 'key_type', 'algorithm', 'length_bits', 'origin', 'usage', 'scope', 'status', 'is_backup_encrypted', 'backup_blob', 'created_by', 'activated_at', 'retired_at', 'replaced_by', 'notes' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\CryptoKeys\Dto\CryptoKeyDto|null */
    public function getByBasenameAndVersion(string $basename, int $version, bool $asDto = false): array|\BlackCat\Database\Packages\CryptoKeys\Dto\CryptoKeyDto|null {
        $row = $this->getByUnique([ 'basename' => $basename, 'version' => $version ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\CryptoKeys\Mapper\CryptoKeyDtoMapper::fromRow($row);
    }
    public function existsByBasenameAndVersion(string $basename, int $version): bool {
        $where = 't.' . Ident::q($this->db, 'basename') . ' = :uniq_basename' . ' AND ' . 't.' . Ident::q($this->db, 'version') . ' = :uniq_version';
        return $this->exists($where, [ 'uniq_basename' => $basename, 'uniq_version' => $version ]);
    }
    /** @return int|string|null */
    public function getIdByBasenameAndVersion(string $basename, int $version) {
        $row = $this->getByBasenameAndVersion($basename, $version);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\CryptoKeys\Dto\CryptoKeyDto|null */
    public function getByConstraint(mixed $constraint, bool $asDto = false): array|\BlackCat\Database\Packages\CryptoKeys\Dto\CryptoKeyDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\CryptoKeys\Dto\CryptoKeyDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\CryptoKeys\Dto\CryptoKeyDto|null {
