...
use BlackCat\Database\Packages\CryptoKeys\Repository\CryptoKeyRepositoryInterface;
...
class CryptoKeyRepository implements CryptoKeyRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'basename', 'version' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\CryptoKeys\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'filename', 'file_path', 'fingerprint', 'key_meta', 'key_type', 'algorithm', 'length_bits', 'origin', 'usage', 'scope', 'status', 'is_backup_encrypted', 'backup_blob', 'created_by', 'activated_at', 'retired_at', 'replaced_by', 'notes' ]);
          return count($rows);
        }
    ...
    public function getByBasenameAndVersion(string $basename, int $version, bool $asDto = false): array|\BlackCat\Database\Packages\CryptoKeys\Dto\CryptoKeyDto|null {
        return $this->getByUnique([ 'basename' => $basename, 'version' => $version ], $asDto);
    }
    ...
    public function getIdByBasenameAndVersion(string $basename, int $version) {
        $row = $this->getByBasenameAndVersion($basename, $version, false);
